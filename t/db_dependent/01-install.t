...
subtest("Scenario: Simple plugin lifecycle tests.", sub {
    ...
    subtest("Make sure the plugin is uninstalled", sub {
        ...
        #We cannot use
        #  Koha::Plugins::Handler->delete({class => 'Koha::Plugin::Fi::Hypernova::ValueBuilder'}); #This actually installs and deletes the plugin.
    ...
    subtest("Upgrade the plugin", sub {
        ...
        $plugin = Koha::Plugin::Fi::Hypernova::ValueBuilder->new(); #This implicitly calls upgrade()
    ...
    subtest("Configure the plugin", sub {
        ...
        $plugin->{cgi}->{save} = 1;
        $plugin->{cgi}->{pattern_itemcallnumber} = "11:11";
        ok($plugin->configure(), "Saving configurations");

        like($plugin->retrieve_data('pattern_itemcallnumber'), qr/11:11/, "pattern_itemcallnumber set");
    });
